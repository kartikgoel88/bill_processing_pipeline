...
class OpenAICompatibleClient:
    ...
    def _is_ollama(self) -> bool:
        """True if base_url is likely Ollama (localhost:11434 or ollama)."""
    ...
    def chat(
        ...
        *,
        ...
        # Ollama can return 400 when top_p is sent (version/model dependent); omit for Ollama
        if top_p is not None and not self._is_ollama():
            payload["top_p"] = top_p
        ...
        if not resp.ok:
            ...
            try:
                err_json = resp.json()
                err_body = str(err_json.get("error", err_json.get("message", err_body)))
            except Exception:
                pass
            logger.error(
                ...
                resp.status_code,
                url,
                (err_body[:500] if err_body else resp.reason),
            )
